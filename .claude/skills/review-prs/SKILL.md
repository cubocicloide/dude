...
| 4 | **Docs triad** | A command added/changed/removed → the CLAUDE.md command reference **and** the stack's `templates/base/docs/` **and** the root `docs/` site. Missing any one is a real finding. |
...
| 6 | **`minDudeVersion`** | A stack using a newly added core API must raise its `minDudeVersion`, or older CLIs fail at runtime. |
