...
describe('api sync', () => {
  it('fetches the drf-spectacular YAML schema from /api/schema/ and generates the client', async () => {
    const spec = makeSpec()
    const fetchMock = vi
      .fn()
      .mockResolvedValue({ ok: true, text: async () => yaml.stringify(spec) })
    ...
    // The spec is fetched from the drf-spectacular schema endpoint.
    expect(fetchMock).toHaveBeenCalledWith('http://localhost:8000/api/schema/')
  ...
  it('parses a JSON schema response too', async () => {
    const spec = makeSpec()
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue({ ok: true, text: async () => JSON.stringify(spec) }),
    )
    captureIO()
    const root = seededProject()
  ...
  it('honors a custom --url override', async () => {
    ...
    await syncCommand.run!(makeCtx(root, { url: 'http://api.example.com:9999/' }))
    expect(fetchMock).toHaveBeenCalledWith('http://api.example.com:9999/api/schema/')
  })
...
async function buildCleanTree() {
  ...
  vi.stubGlobal(
    'fetch',
    vi.fn().mockResolvedValue({ ok: true, text: async () => yaml.stringify(spec) }),
  )
  const io = captureIO()
  const root = seededProject()
  await syncCommand.run!(makeCtx(root))
