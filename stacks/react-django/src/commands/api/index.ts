...
export const syncCommand: StackCommandDef = {
  description:
    'Fetch the OpenAPI schema from the Django backend (drf-spectacular) and generate a typed client in frontend/src/openapi/.',
  args: {
    url: { type: 'string', description: 'Backend base URL.', default: 'http://localhost:8000' },
    out: {
      type: 'string',
  ...
  async run({ projectRoot: root, args }) {
    ...
    // drf-spectacular's SpectacularAPIView; serves YAML by default and JSON
    // when content negotiation asks for it — accept either.
    const specUrl = `${url.replace(/\/$/, '')}/api/schema/`
    ...
    try {
      ...
      spec = (body.trimStart().startsWith('{')
        ? JSON.parse(body)
        : yaml.parse(body)) as OpenAPI3
    } catch (err) {
      process.stderr.write(`error: could not fetch OpenAPI spec — ${(err as Error).message}\n`)
      process.stderr.write(`Make sure the Django backend is running at ${url}\n`)
      process.exit(1)
    }
