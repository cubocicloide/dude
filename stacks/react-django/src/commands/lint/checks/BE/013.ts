...
interface PyClass {
  ...
  line: number
  body: BodyLine[]
  /** Contiguous non-blank lines directly above the class def (decorators live here). */
  preamble: string[]
}
...
/** Parse `class X(...):` definitions with bodies and preamble; multi-line-base aware. */
function parseClasses(lines: string[]): PyClass[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    classes.push({ name: m[2]!, bases, line: i + 1, body, preamble })
  }
  return classes
}
...
const EXTEND_SCHEMA_RE = /^\s*@extend_schema(_view)?\b/

/** BE013 — views must declare serializer_class or carry @extend_schema for a typed OpenAPI schema */
export default function check(root: string): RawDiagnostic[] {
  ...
  walkFiles(appsDir, 'views.py', (abs, rel) => {
    ...
    for (const cls of parseClasses(content.split('\n'))) {
      ...
      const hasSerializer = cls.body.some((b) => /^\s*serializer_class\s*[:=]/.test(b.text))
      const decoratedClass = cls.preamble.some((l) => EXTEND_SCHEMA_RE.test(l))
      const decoratedMethod = cls.body.some((b) => EXTEND_SCHEMA_RE.test(b.text))

      if (!hasSerializer && !decoratedClass && !decoratedMethod) {
        diagnostics.push({
          file: fileRel,
          line: cls.line,
          col: 1,
          severity: 'warning',
          message: `View class \`${cls.name}\` declares no serializer_class and no @extend_schema — the OpenAPI schema (and generated TS client) will be untyped`,
        })
      }
    }
  })
