...
const RELATION_RE = /\b(ForeignKey|OneToOneField|ManyToManyField)\s*\(/g

/** BE011 — relation fields must pass an explicit related_name= */
export default function check(root: string): RawDiagnostic[] {
  ...
  walkFiles(appsDir, 'models.py', (abs, rel) => {
    ...
    lines.forEach((line, idx) => {
      ...
      for (const m of line.matchAll(RELATION_RE)) {
        const openCol = line.indexOf('(', m.index)
        if (openCol === -1) continue
        const span = argSpan(lines, idx, openCol)
        if (!/\brelated_name\s*=/.test(span)) {
          diagnostics.push({
            file: fileRel,
            line: idx + 1,
            col: m.index + 1,
            severity: 'warning',
            message: `${m[1]} should pass an explicit related_name= — implicit \`<model>_set\` reverse accessors are brittle`,
          })
        }
      }
    })
