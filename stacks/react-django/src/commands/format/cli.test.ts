/**
 * CLI integration test: `dude format` (react-django stack)
 */
...
describe('dude format', () => {
  beforeAll(() => {
    project = Project.scaffold({ stack: './stacks/react-django', prefix: 'dude-format-' })
  ...
  it.skipIf(!UV_AVAILABLE)('format --check exits 1 on a badly-formatted .py file', () => {
    project.write('backend/apps/bad.py', '"""Bad formatting."""\nx=1\ny   =   2\n')
    expect(project.run('format', '--check').status).toBe(1)
    project.restore('backend/apps/bad.py')
  })

  it.skipIf(!UV_AVAILABLE)('format (write) fixes a badly-formatted .py file', () => {
    project.write('backend/apps/bad.py', '"""Bad."""\nx=1\n')
