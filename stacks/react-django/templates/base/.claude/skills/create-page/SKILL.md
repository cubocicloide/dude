...
> Read `.claude/rules/FE/001.md`–`012.md` once at the start — they are the
> source of truth. The stack uses **React 19 + Vite + TypeScript (strict)**,
> **Ant Design** (`antd`) for UI, **@tanstack/react-query** for data,
> **zustand** for global client state, and **react-router-dom** for routing.
...
1. **Route path** — e.g. `/todos`, `/users/[id]`. Each segment becomes a
   directory under `pages/` (kebab-case, or `[param]` for a dynamic segment)
   and the `<Route path>` in `App.tsx` (`[id]` → `:id`).
...
# Reusable hooks (usePageTitle, zustand stores) + barrel
...
- **Hooks to reuse** — always wire `usePageTitle(<title>)`; reuse any `use*`
  hook or zustand store that already holds the state you need.
...
- **Sub-components to introduce** — if the page has distinct reusable chunks,
  plan them as page-local `$components/` (used only here) or shared
...
| File | Rule | Purpose |
|------|------|---------|
| `pages/<route>/index.tsx` | FE004, FE005 | the page component (mandatory) |
| `pages/<route>/styles.module.css` | FE005 | optional scoped styles |
| `pages/<route>/types.tsx` | FE005, FE012 | optional local types |
| `pages/<route>/constants.tsx` | FE005, FE012 | optional local constants |
| `pages/<route>/functions.tsx` | FE005, FE012 | optional local helpers |
| `pages/<route>/$components/<Comp>/index.tsx` | FE001, FE002 | optional page-local sub-components (+ barrel `$components/index.tsx`, FE003) |
| `pages/<route>/$hooks/use<Name>/index.tsx` | FE006, FE007 | optional page-local hooks (+ barrel) |
| `src/App.tsx` | FE004 | add `import X from '@/pages/<route>'` **and** a `<Route path="<path>" element={<X />} />` |
| `src/$components/<Comp>/` + barrel | FE001–003 | only when introducing a **shared** component |
| `src/$hooks/use<Name>/` + barrel | FE006, FE007 | only when introducing a **shared** hook or zustand store |
| `<scope>/$assets/...` | FE008 | any new static asset lives in the `$assets/` of the scope that uses it |

Allowed contents reminders:
- Any **scope dir** (page, component, hook, utils domain) holds only
  `index.tsx`, `styles.module.css`, `types.tsx`, `constants.tsx`,
  `functions.tsx` plus its privileged `$`-folders (FE002/FE005/FE006/FE009).
- **Route segments** are kebab-case or `[param]` — the `$` prefix keeps
  structural folders (`$components/`, `$hooks/`, `$assets/`, `$misc/`) out of
  the route namespace (FE005).
- A **component dir** is PascalCase (FE001) and re-exported from its parent
  `$components/index.tsx` barrel in alphabetical order (FE003).
- A **hook dir** matches `use[A-Z]...` and is re-exported from its
  `$hooks/index.tsx` barrel (FE006/FE007).
- Keep `index.tsx` thin (FE012): types → `types.tsx`, helpers →
  `functions.tsx`, extra components → `$components/`, hooks → `$hooks/`.
- Never park files in `$misc/` if you know where they belong — every `$misc/`
  raises a warning (FE011).
...
If the page should appear in the sidebar, add a matching entry to `MENU_ITEMS`
in `$components/Layout/constants.tsx` (key = first path segment, with an
...
- Global client state lives in a zustand store under `$hooks/` — mirror
  `$hooks/useCounterStore/` (devtools middleware, `enabled: import.meta.env.DEV`).
- Imports use the `@/` alias (assets are imported from their scope's `$assets/`, FE008).
- Adding a component → update its `$components/index.tsx` barrel; adding a hook
  → update the `$hooks/index.tsx` barrel in the same change (FE003/FE007).
...
Page created
═════════════════════════════════════════
Route       <path>  →  pages/<route>/index.tsx
