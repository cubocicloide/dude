...
export interface EcsTarget {
  /** ECR repository URL for the server image. */
  repository: string
  registryHost: string
  region: string
  cluster: string
  service: string
  /** Public MCP endpoint, when the infra reports one. */
  mcpUrl?: string
}
...
export function readEcsTarget(projectRoot: string, profile?: string): EcsTarget | null {
  ...
  try {
    ...
    return {
      ...
      cluster,
      service,
      mcpUrl: o.mcp_url?.value || undefined,
    }
  } catch {
    return null
  }
...
/**
 * Build the production image, tagged for the shared ECR repository.
...
export function doBuild(
  ...
  process.stdout.write(`\n  → building server  ${target.repository}:${tag} (${platform})\n`)
  return run(
    'docker',
    [
      'build',
...
/** Log in to ECR and push the server image at `:tag`. */
...
/**
 * Roll the ECS service onto `tag`: record it as `image_tag` in the env's
 ...
 * version), then `terraform apply` — which registers a new task definition
 * revision and lets ECS roll the service with its circuit breaker armed.
 */
export function doDeploy(
  ...
  if (code === 0) {
    process.stdout.write(
      `\n  ✓  Task definition updated to :${tag} — ECS is rolling the service now\n` +
        `     (auto-rollback on failure). Watch it with: dude iac status --env ${env}\n` +
        (target.mcpUrl ? `     MCP endpoint: ${target.mcpUrl}\n` : ''),
    )
  }
  return code
}
