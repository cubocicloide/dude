...
export interface AdapterOpts {
  ...
  cachePrefix: string
  /** Override the Docker image tag for `trivy-image`. Defaults to `<cachePrefix>-fastmcp:latest`. */
  targetImage?: string
}
...
/** Classify a finding's component by its repo-relative path. */
function componentOf(filePath: string): string {
  if (filePath.startsWith('fastmcp/')) return 'fastmcp'
...
export const banditAdapter: Adapter = {
  ...
  buildCommand(projectRoot, runDir, opts) {
    ...
    const banditCmd =
      ...
      `bandit -r fastmcp/app -f json -o ${report} ` +
      "--exclude 'fastmcp/app/tests'"
    return [
      'docker',
      'run',
      '--rm',
  ...
  parse(data) {
    ...
    for (const item of results) {
      ...
      findings.push({
        ...
        component: componentOf(filePath),
        extra: {
...
// Curated rulesets for a Python (FastMCP) service.
const SEMGREP_CONFIGS = ['p/security-audit', 'p/python', 'p/dockerfile']
...
export const trivyImageAdapter: Adapter = {
  ...
  buildCommand(projectRoot, runDir, opts) {
    ...
    const targetImage =
      opts.targetImage ?? process.env['TRIVY_TARGET_IMAGE'] ?? `${opts.cachePrefix}-fastmcp:latest`
    return [
      'docker',
      'run',
      '--rm',
