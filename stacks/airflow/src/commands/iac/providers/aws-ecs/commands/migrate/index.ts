/** `dude iac migrate` — initialize/upgrade the Airflow metadata DB as a one-off Fargate task. */
...
function readMigrateTarget(projectRoot: string, profile: string): MigrateTarget | null {
  ...
  try {
    ...
    const subnets = Array.isArray(o.public_subnet_ids?.value)
      ...
      : []
    const securityGroup = String(o.service_security_group_id?.value ?? '')
...
export const iacMigrateCommand: StackCommandDef = {
  available: hasIac,
  description:
    'Initialize/upgrade the Airflow metadata database (airflow db migrate) as a one-off ECS Fargate task. Run once after the first apply, and after every Airflow version bump.',
  args: { ...envArg },
  async run({ projectRoot, args }) {
    ...
    process.stdout.write(`\n  → migrating the metadata DB on "${env}" (${target.taskDefinition})…\n`)
    const netConf =
