...
const REQUIRED_DIRS = [
  'core', 'fixtures', 'management', 'models', 'queries', 'routers', 'schemas', 'tests', 'utils',
]
  ...
  if (existsSync(testsDir)) {
    ...
    for (const dir of REQUIRED_TESTS_DIRS) {
      if (!existsSync(path.join(testsDir, dir))) {
        diagnostics.push({ file: testsRel, line: 1, col: 1, severity: 'error', message: `backend/app/tests/${dir}/ is missing` })
      } else if (!existsSync(path.join(testsDir, dir, '__init__.py'))) {
        diagnostics.push({ file: path.join(testsRel, dir), line: 1, col: 1, severity: 'error', message: `backend/app/tests/${dir}/__init__.py is missing` })
      }
    }
    for (const file of REQUIRED_TESTS_FILES) {
      if (!existsSync(path.join(testsDir, file))) {
        diagnostics.push({ file: testsRel, line: 1, col: 1, severity: 'error', message: `backend/app/tests/${file} is missing` })
      }
    }
  }
