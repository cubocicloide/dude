/** `dude iac bootstrap` — create the Terraform state backend (S3 + DynamoDB). */
...
export const iacBootstrapCommand: StackCommandDef = {
  available: hasIac,
  description:
    'One-time: create the S3 state bucket + DynamoDB lock table, then write the values into backend.hcl.',
  args: {
    region: {
      type: 'string',
