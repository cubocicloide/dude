...
- Stack: FastAPI backend (`backend/`), React + Vite + TypeScript frontend (`frontend/`)
...
- FastAPI routes live in `backend/app/` — follow the existing router/dependency pattern
- Do not break existing endpoint signatures without updating callers
- After model or schema changes, check whether Pydantic schemas in `backend/app/` need updating
