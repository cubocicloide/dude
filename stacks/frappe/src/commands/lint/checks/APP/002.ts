...
/**
 * APP002 — scheduler_events handlers resolve.
 *
 * The scheduler resolves dotted paths at runtime: a typo does not fail at
 * install time, the job just never runs. Verify every handler declared in
 * hooks.py scheduler_events exists on disk.
 */
export default function check(root: string): RawDiagnostic[] {
  const diagnostics: RawDiagnostic[] = []
  for (const app of listApps(root)) {
    diagnostics.push(...checkDottedHandlers(root, app, 'scheduler_events'))
  }
  return diagnostics
}
