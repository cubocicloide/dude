 ...
 * facing ALB whose single target is the nginx `frontend` service (frappe's
 * nginx config proxies API and `/socket.io` traffic itself, reaching the
 * `backend`/`websocket` services via Cloud Map service discovery), five ECS
 * Fargate services all running the ONE production Frappe image in different
 * roles (frontend/backend/websocket/worker/scheduler), an RDS MariaDB
 * instance + ElastiCache Redis (credentials in Secrets Manager) and an EFS
 * file system holding the shared frappe `sites/` volume. One-off Fargate task
 * definitions cover the bench configuration + first-deploy site creation
 * (`dude iac create-site`) and `bench migrate` (`dude iac migrate`).
 *
...
import { iacCreateSiteCommand } from './commands/create-site/index.js'
...
export const awsEcsProvider: IacProvider = {
  ...
  commands: {
    ...
    ship: iacShipCommand,
    'create-site': iacCreateSiteCommand,
