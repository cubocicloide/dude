/** Terraform / project plumbing for the AWS ECS provider. */
...
/** True when the project was scaffolded with the AWS ECS target (`iac/terraform` present). */
  ...
  if (!hasIac(projectRoot)) {
    process.stderr.write(
      '\n  ✗  No IaC configuration found (iac/terraform).\n' +
        ...
        '     Re-scaffold with `--iac aws-ecs` to enable it.\n\n',
    )
    return false
  }
  return true
